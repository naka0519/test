################### openface書き込み開始時間の取得と同期スタートフレーム番号リストの返り値 ##################
import requests
import zipfile
import io
import os
...
def main():
    ...
    # 時間からフレーム差と番号を計算
    #print(w_time) ####### ほぼ差がないことがわかる
    start_frame = [0 for _ in range(num)]
    max_time = max(w_time)
    print(max_time)
    dif_time = [w_time[i] - max_time for i in range(num)]


    return start_frame # 各カメラの初めframe番号を返す
