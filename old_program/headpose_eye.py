...
def pre_process():
    ...
    for i in range(cam_num):
        ...
        with open(csvpath, "w") as f: # csvを白紙に(データ削除) カラムの設定だけしたい
            #f.write("")
    ...
    for i in range(len(csv_list)):
        if forward_list[i].values == []: # 繰越されるframeがあるかないかを判定する(データを持たないもの) (os.stat(forwardpath_list[i]).st_size == 0) or 
    ...
    # frame数の把握と調整    : 一回目で開始時のframeを合わせる必要性がある
    ...
    for i in range(len(frame_list)):
        ...
        forward_csv.to_csv("/home/ubuntu/workdir/openface/OpenFace-OpenFace_2.2.0/build/bin/rtsp/r_cam{}_forward.csv".format(len(csv_list) - i), mode = "w") 
    #'''
    ...
    df.to_csv("./headpose_result/testconcat.csv", mode = "w") # 記録用
...
def main():
    '''
    # headposeの出力ファイル作成
    p_dir = pathlib.Path(/home/ubuntu/workdir/processe/headpose_result)
    with p_dir.open("test.csv", "w") as f:
        f.write("")
    '''

    '''
    # 標準入力化からrtspのデータを取得する時

    # tail -f .csvの行数を確認する
    n = 5

    # tailの新規情報を標準入力として受けとる
    face_res = []
    ...
    #for item in face_res:
    #    print(item)
    #print(face_res[0])

    # Dataframe型に変換して、座標変換を行う
    face_res = pd.DataFrame(face_res)
    df = face_res.iloc[1:]
    df.columns = face_res.iloc[0]
    df = df.astype("float")
    #print(df.iloc[0])
    ...
    for i in range(num):
        ...
        if ((df.iloc[i][" success"] < 0.75)): #or (df.iloc[i][" pose_Rx"] >= 0.9) or (df.iloc[i][" pose_Ry"] >= 0.9)):
            ...
            S_data = pd.DataFrame(data = [[frame, cam_id, face_id, timestamp, confidence, success, head_loc_x, head_loc_y, head_loc_z, head_rot_x, head_rot_y, head_rot_z, eye_rot_x, eye_rot_y, eye_rot_z, dif_xy, dif_xz]], columns = column)
        ...
        # 推定が成功した時
            ...
            # 目線の向き
            # カメラ基準の目の向き
            c_ex = df.iloc[i][" gaze_angle_y"] * 90.0 # 上下
            c_ey = df.iloc[i][" gaze_angle_x"] * 90.0 * (-1) # 左右
            c_ez = 0.0
            
            eye_rot_x = c_ez
            eye_rot_y = camera_gaze[cam][1] + 180 + c_ex
            eye_rot_z = camera_gaze[cam][2] + 180 - c_ey
            
            # 目線と顔の向きの差異
            # xy平面
            dif_xy = round(abs(head_rot_z - eye_rot_z), 4)
            # xz平面
            dif_xz = round(abs(head_rot_y - eye_rot_y), 4)
        
            # 三次元上の変移角
            '''
            vec_head = [1, y[0] - y[1], z[0] - z[1]]
            vec_eye = [1, ye[0] - ye[1], ze[0] - ze[1]]
    ...
    #result.to_csv("/home/ubuntu/workdir/processe/headpose_result/trans{}.csv".format(time), mode = "w")
    result.to_csv("/home/ubuntu/workdir/processe/headpose_result/trans.csv", mode = "w")
